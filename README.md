...
| 항목 | 기준 | 결과 |
|---|---|---|
| 시험 | 실패 0 | **657 통과** (Java) · **564 통과** (C# 이식본) |
| 라인 / 분기 커버리지 | ≥ 90 % / ≥ 85 % | Java **93.7 % / 92.6 %** (JaCoCo) · C# **92.3 % / 93.3 %** (coverlet) |
| 뮤테이션 검출률 (PIT STRONGER) | ≥ 80 % | **94.0 %** (280/298) — 생존 18개 전부 판정 근거 기록 |
...
| PMD · SpotBugs | 0 | 0 · 0 |
| SonarQube Cloud (품질 게이트) | 통과 | **통과** — 버그 0 · 취약점 0 · 중복 0.0 % · 등급 A · 코드 스멜 6 (데모 출력 3 · NaN 검사 2 · 복잡도 1) ([대시보드](https://sonarcloud.io/dashboard?id=Haejyn_orbit-pass-sim)) |
| 성능 (`PassPredictor.predict`, 10 s 간격, JDK 26) | 표본당 궤적 평가 ≤ 1.10 (회귀 게이트) | 이체 하루 창 **2.3 ms** · 30 일 창 **92 ms** (표본당 약 0.3 µs), J2 는 약 1.4 배. 표본당 궤적 평가 **1.02~1.03 회** ([보고서 §10](docs/test-report.md)) |
